...
class DateHourPickerModel extends DatePickerModel {
  ...
  DateHourPickerModel({DateTime currentTime,
    DateTime maxTime,
    DateTime minTime,
    LocaleType locale})
      : super(locale: locale) {
  ...
  void _fillRightEndLists() {
    ...
    int minHour = _minHourOfCurrentDay();
    this.rightEndList = List.generate(maxHour - minHour + 1, (int index) {
      int _hour = minHour + index;
      return "${_hour < 10 ? '0$_hour' : '$_hour'}:00";
    });
  }
...
class DateAmPmPickerModel extends DatePickerModel {
  ...
  DateAmPmPickerModel({DateTime currentTime,
    DateTime maxTime,
    DateTime minTime,
    LocaleType locale,
    bool isAm = true})
    ...
    rightEndList = ["上午", "下午"];
    _currentRightEndIndex = this.isAm ? 0 : 1;
  }
...
class DateHalfHourPickerModel extends CommonPickerModel {
  ...
  DateHalfHourPickerModel({DateTime currentTime,
    DateTime maxTime,
    DateTime minTime,
    LocaleType locale})
      : super(locale: locale) {
    ...
    if (currentTime.hour == 8) {
      rightEndList = ['00', '15', '30'];
      if (this.currentTime.minute >= 0 && this.currentTime.minute < 15) {
        _currentRightIndex = 0;
      } else if (this.currentTime.minute > 15 && this.currentTime.minute < 30) {
        _currentRightIndex = 1;
      } else {
        _currentRightIndex = 2;
      }
    } else {
      rightEndList = ['00', '30'];
  ...
  void setRightIndex(int index) {
    ...
    currentTime = currentTime.isUtc
      ...
      currentTime.day,
      currentTime.hour,
      index == 0 ? 0 : (currentTime.hour == 8 ? (index == 1 ? 15 : 30) : 30),
    )
        : DateTime(
      currentTime.year,
      currentTime.month,
  ...
  DateTime finalTime() {
    ...
    var minute = _currentRightIndex == 0
        ? 0
        : (hour == 8 ? (_currentRightIndex == 1 ? 15 : 30) : 30);
